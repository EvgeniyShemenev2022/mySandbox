...
CopyOnWriteArrayList следует использовать, когда необходимо обеспечить потокобезопасность. В алгоритме небольшое 
количество изменений(вставок) и большое количество по чтению эл-в.

При операциях, изменяющих содержимое коллекции, происходит полное копирование списка (новый при каждой операции). 
Таким образом лучше ограничиться чтением.
